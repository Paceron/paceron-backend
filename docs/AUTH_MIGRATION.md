...
**Implicancia concreta a tener presente**: `POST /api/v1/users/:id/roles` cae en esta misma categoría — cualquier usuario logueado puede auto-asignarse el rol `entrenador` (o asignárselo a cualquier otro) con una sola llamada. Como casi todas las reglas "solo entrenador" de la sección 4 dependen exclusivamente de tener ese rol, esto significa que **cualquier corredor puede auto-promoverse a entrenador y saltear todos esos chequeos**. No es un bug de esta iniciativa — es la misma limitación de "no existe admin" aplicada a su caso más sensible. Si en algún momento se decide cerrar esto, hace falta primero definir quién puede otorgar el rol `entrenador` (¿aprobación manual? ¿verificación externa?), que es una decisión de producto, no solo técnica.
