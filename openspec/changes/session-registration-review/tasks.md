...
- [x] 1.1 Crear `cmd/api/domains/dbs/runner_session.go` con `RunnerSession`:
...
- [x] 1.2 Agregar `&dbs.RunnerSession{}` al `AutoMigrate` de
      `cmd/api/infrastructure/postgresdb/postgres.go`.
- [x] 1.3 Verificar que AutoMigrate cree el constraint único `uq_runner_session_session_athlete`.

## 2. DTOs

- [x] 2.1 Crear `cmd/api/domains/runnersession/runner_session.go`:
...
- [x] 2.2 Constantes `MsgRunnerSessionCreated` / `MsgRunnerSessionFinished`.

## 3. DAO runner_session

- [x] 3.1 Crear `cmd/api/daos/runner_session_dao.go` con
...
- [x] 3.2 Errores: `ErrRunnerSessionNotFound = errors.New("estado de sesión no encontrado")`.
- [x] 3.3 `cmd/api/daos/workout_feedback_dao.go`: agregar `GetBySession(ctx, sessionInstanceID,
...
- [x] 4.1 Crear `cmd/api/services/runner_session_service.go` con
...
- [x] 4.2 `cmd/api/services/workout_feedback_service.go`: `GetSessionFeedback(ctx, authUserID,
...
- [x] 5.1 Crear `cmd/api/controllers/runner_session_controller.go` (`Create` 201/200, `Finish` 200,
...
- [x] 5.2 `workout_feedback_controller.go`: `GetBySession` (200/400/401/403), reusa
      `respondFeedbackError` + `toWorkoutFeedbackResponse` (para `data` usa el shape de lista).
- [x] 5.3 Swagger annotations godoc para las 4 rutas.
- [x] 5.4 DI en `cmd/api/app/app.go`: `runnerSessionDao := daos.NewRunnerSessionDao(db)`,
...
- [x] 5.5 Registrar en `cmd/api/app/url_mappings.go`:
      `POST/PATCH/GET /api/v1/session-instances/:id/runner` y
      `GET /api/v1/session-instances/:id/feedback`.
- [x] 5.6 Regenerar swagger (`make swagger` o el comando del repo).

## 6. Tests

- [x] 6.1 DAO test `runner_session_dao_test.go` (mocks/GORM real según patrón del módulo):
...
- [x] 6.2 `workout_feedback_dao_test.go`: `GetBySession` filtra/ordena y excluye soft-deleted.
- [x] 6.3 Service test `runner_session_service_test.go`: sesión inexistente (404), start_date vacío
      (400), atleta ajeno sin relación (403) / con relación (ok), idempotencia de create y de
      finish, end_date seteada por servidor.
- [x] 6.4 Service test feedback: matriz self/trainer/no-auth + orden.
- [x] 6.5 Controller test: 200/201/400/403/404 de las 4 rutas.
- [x] 6.6 `go test ./...` verde; cobertura del repo >= 85%.

## 7. Docs

- [x] 7.1 Swagger regenerado en `cmd/api/docs/`.
- [x] 7.2 Nota en `CLAUDE.md` del backend: `runner_session` = estado de dominio del corredor
      (wip→finished), FK opaca + UNIQUE real, creación idempotente.
- [x] 7.3 Copiar spec aprobada a `openspec/specs/session-registration-review/spec.md`.