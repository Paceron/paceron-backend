...
- [x] 3.1 Agregar `GET /api/v1/users/:id/roles` y `DELETE /api/v1/users/:id/roles/:role_id` en `app/url_mappings.go`
- [x] 3.2 Regenerar Swagger, actualizar tabla de endpoints en `README.md`
- [x] 3.3 Agregar requests a las colecciones Bruno (`get roles v1`, `remove role v1`)

## 4. Verificación

- [x] 4.1 `go build ./...`, `go vet ./...`, `go test ./...` — todo verde
- [ ] 4.2 Probar manualmente: asignar un rol, listarlo (`GET`), darlo de baja (`DELETE`), volver a listar y confirmar que ya no aparece — pendiente, requiere servidor corriendo, a cargo del usuario
