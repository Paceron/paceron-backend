...
type GroupUserServiceInterface interface {
	AddUser(ctx *gin.Context, teamID, groupID int64, req *groupuser.AddGroupUserRequest) (*groupuser.GroupUserResponse, error)
	RemoveUser(ctx *gin.Context, groupID, userID int64) error
...
// AddUser agrega un usuario a un grupo.
// Valida que el grupo pertenezca al equipo, que el usuario exista y que no esté ya asociado.
func (s *groupUserService) AddUser(ctx *gin.Context, teamID, groupID int64, req *groupuser.AddGroupUserRequest) (*groupuser.GroupUserResponse, error) {
...
// RemoveUser quita un usuario de un grupo (soft-delete de la asociación).
func (s *groupUserService) RemoveUser(ctx *gin.Context, groupID, userID int64) error {
	...
	existing, err := s.groupUserDao.FindByGroupAndUser(ctx, groupID, userID)
	if err != nil {
