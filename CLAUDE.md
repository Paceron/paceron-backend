...
- **GitHub Actions dejó de correr desde el 2026-08-03**: ni `auto-pr.yml` ni `ci.yml` se dispararon para push/PR/merge en la PR #23 (2026-08-06) pese a workflows activos, permisos del repo en "all", cuota de minutos en 0/2000 (no es billing). Revisado a mano en la UI de GitHub sin encontrar la causa. La PR #23 se mergeó igual, verificada en verde a mano (`go test ./...` local) ya que no hubo check automático. Si esto persiste, confirmar con GitHub Support o revisar de nuevo antes de asumir que CI está protegiendo los merges — hoy no lo está haciendo.
