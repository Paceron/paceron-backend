...
`ci.yml` mide coverage en cada push/PR y además escribe un resumen (`go tool cover -func`) al Job Summary del run — visible directo en la pestaña de checks de GitHub, sin abrir logs. El gate lo hace la action `vladopajic/go-test-coverage`, configurada en `.testcoverage.yml` — **`threshold.total: 85`, bloquea el merge si el coverage cae por debajo.** Alcanzado agregando Postgres real en CI para testear `daos` (antes ~4%, ahora >85% ese paquete; total del proyecto ~85%). No bajar el número real del proyecto para pasar el gate — si baja, agregar tests, no ajustar el umbral hacia abajo.
...
- `workout_feedback_points` (change `workout-feedback-gps-points`) persiste el recorrido GPS de cada serie: `feedback_id` es FK **opaca** a `workout_feedback` (sin cascade — un hipotético borrado no arrastra el historial), `session_instance_id`/`exercise_instance_id` van **denormalizadas** (consulta por sesión/ejercicio sin join) y `"order"` es el ordinal 0-based del punto dentro de la serie. La idempotencia del bulk `POST /workout-feedback/:id/points` la da la DB: índice único `uq_feedback_point_order (feedback_id, "order")` + `ON CONFLICT DO NOTHING` → `{ created, skipped }`; `"order"` se cita en SQL crudo (palabra reservada).
- `runner_session` (change `session-registration-review`) es el **estado de dominio** del corredor para una sesión asignada: `status` `wip` (se dio Play o se entró al registro manual) → `finished` (micro-endpoint `POST /session-instances/:id/runner` para wip, `PATCH` del mismo path para finished). `session_instance_id`/`athlete_user_id` son FK **opacas**, pero la **unicidad es real y plana**: índice único `uq_runner_session_session_athlete` (no parcial) — un atleta tiene un solo estado por sesión y dos atletas comparten la sesión en un día de grupo. La creación es **idempotente** vía `ON CONFLICT DO NOTHING`: reintentar el wip no duplica ni pisa (devuelve `created=false` y el estado actual, 200). `end_date` la setea el servidor al pasar a `finished`, y el UPDATE lleva `WHERE status='wip'` como guard: una sesión ya finalizada nunca se reescribe.
